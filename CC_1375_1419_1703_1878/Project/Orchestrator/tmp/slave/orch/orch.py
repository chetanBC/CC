...
def create_slave(i):
    ...
    container=client.containers.run(image="worker:latest",name=slave_name,links={'rmq':'rmq'}, network_mode='tmp_default', detach=True)
...
def creat_cond2():
    ...
    while(num<0 and n1<0):
        n1+=1
        req=requests.post(url="http://ec2-54-159-120-199.compute-1.amazonaws.com/api/v1/crash/slave")
        rep=req.json()
        print(rep)
